...
import { getCurrentUser, getUserId, getDisplayName, getAvatarUrl, getAuthHeaders, hasValidLoginSession, saveCurrentUser, updateCurrentUser, handleAuthError } from "./profile-auth.js";
/* =========================================================
   CONFIG
   ========================================================= */
...
/* =========================================================
   RENDER AVATAR
   ========================================================= */
function renderAvatar(element, name, image) {
    if (!element) {
        return;
    }
    element.replaceChildren();
    const imageUrl = (typeof image ===
        "string"
        &&
            image.trim())
        ? image.trim()
...
function renderProfileAvatars() {
    ...
    renderAvatar(editAvatar, name, avatar);
}
/* =========================================================
...
function validateImageFile(file) {
    ...
    if (file.size >
        ...
        return {
            valid: false,
            error: "Please choose an image smaller than 3 MB."
        };
    }
    return {
        valid: true,
            ...
            (event) => {
                ...
                image.onload =
                    () => {
                        ...
                        if (largestDimension >
                            MAX_IMAGE_DIMENSION) {
                            const scale = MAX_IMAGE_DIMENSION /
                                largestDimension;
                            width =
                                Math.round(width *
                                    scale);
...
/* =========================================================
   BUTTON STATE
   ========================================================= */
function setChangePhotoButtonState(disabled, text = "Change photo") {
    if (!changePhotoBtn) {
        return;
    }
...
function setRemovePhotoButtonState(disabled, text = "Remove") {
    if (!removePhotoBtn) {
        return;
    }
...
async function uploadProfilePhoto(file) {
    ...
    if (!userId) {
        alert("Your account ID is missing. Please login again.");
        return false;
    }
    setChangePhotoButtonState(true, "Uploading...");
    try {
         ...
         */
        window.dispatchEvent(new CustomEvent("dheere:profile-avatar-updated", {
            detail: {
                avatarUrl: result.avatarUrl
            }
        }));
        ...
        alert(getErrorMessage(error, "Could not upload the profile photo."));
        return false;
    }
    finally {
        setChangePhotoButtonState(false, "Change photo");
        if (photoInput) {
            photoInput.value =
                "";
        }
...
async function removeProfilePhoto() {
    ...
    setRemovePhotoButtonState(true, "Removing...");
    try {
        const response = await fetch(`${API_BASE}/profile/${encodeURIComponent(userId)}/photo`, {
            method: "DELETE",
            headers: getAuthHeaders()
        ...
        alert(getErrorMessage(error, "Could not remove the profile photo."));
        return false;
    }
    finally {
        setRemovePhotoButtonState(false, "Remove");
    }
}
/* =========================================================
   PHOTO INPUT
...
function setupMediaEvents() {
     ...
     */
    if (editOverlay) {
        editOverlay.addEventListener("transitionend", () => {
     ...
     * profile-editor.js.
     */
    window.addEventListener("dheere:profile-updated", () => {
        renderProfileAvatars();
    });
}
/* =========================================================
...
/* =========================================================
   EXPORTS
   ========================================================= */
export { initializeProfileMedia, renderAvatar, renderProfileAvatars, validateImageFile, compressProfilePhoto, uploadProfilePhoto, removeProfilePhoto };
