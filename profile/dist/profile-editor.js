/* =========================================================
   ...
   - Dheere AI
   ========================================================= */
import { getCurrentUser, getUserId, getUsername, getAuthHeaders, hasValidLoginSession, saveCurrentUser, updateCurrentUser, handleAuthError } from "./profile-auth.js";
/* =========================================================
   CONFIG
   ========================================================= */
const API_BASE = "/api";
...
const PROFILE_DATE_REGEX = /^\d{4}-\d{2}-\d{2}$/;
const PROFILE_USERNAME_REGEX = /^[a-z0-9_]{3,20}$/;
/* =========================================================
   STATE
   ========================================================= */
...
function isValidDateOfBirth(value) {
    ...
    if (!PROFILE_DATE_REGEX.test(value)) {
        return false;
    }
...
/* =========================================================
   USERNAME STATUS
   ========================================================= */
function setUsernameStatus(message, type = "") {
    if (!usernameStatus) {
        return;
    }
    usernameStatus.textContent =
        message ||
            "";
    usernameStatus.classList.remove("available", "taken", "checking");
    if (type) {
...
/* =========================================================
   ...
   ========================================================= */
function setSaveButtonState(disabled, text = "Save Changes") {
    if (!saveProfileBtn) {
        return;
    }
...
function openEditModal() {
    ...
    if (editDateOfBirth) {
        editDateOfBirth.value =
            String(currentUser.dateOfBirth ||
                "");
    }
    if (editGender) {
        editGender.value =
    ...
    setSaveButtonState(false, "Save Changes");
    return true;
}
/* =========================================================
   CLOSE MODAL
...
async function checkUsernameAvailability() {
    ...
    if (!username) {
        usernameAvailable =
            false;
        setUsernameStatus("Username is required.", "taken");
        return;
    }
    /*
    ...
    if (username ===
        ...
        usernameAvailable =
            true;
        setUsernameStatus("✓ Current username", "available");
        return;
    }
    /*
    ...
    if (!PROFILE_USERNAME_REGEX.test(username)) {
        usernameAvailable =
            false;
        setUsernameStatus("Username must be 3–20 characters using lowercase letters, numbers or underscores.", "taken");
        return;
    }
    if (!userId) {
        usernameAvailable =
            false;
        setUsernameStatus("Your account ID is missing.", "taken");
        return;
    }
    const requestToken = ++usernameCheckToken;
    setUsernameStatus("Checking username...", "checking");
    usernameAvailable =
        false;
    try {
        const response = await fetch(`${API_BASE}/check-username/${encodeURIComponent(username)}?userId=${encodeURIComponent(userId)}`, {
            method: "GET",
            headers: {
        ...
        if (response.ok &&
            ...
            usernameAvailable =
                false;
            setUsernameStatus("✕ Username is already taken.", "taken");
            return;
        }
        /*
        ...
        usernameAvailable =
            false;
        setUsernameStatus("Username will be verified when you save.", "checking");
    }
    catch (error) {
        if (requestToken !==
            usernameCheckToken) {
...
function handleUsernameInput() {
     ...
     */
    usernameCheckTimer =
        setTimeout(checkUsernameAvailability, 450);
}
/* =========================================================
   FORM VALIDATION
   ========================================================= */
function validateProfileForm() {
    ...
    if (name.length >
        80) {
        alert("Display name cannot exceed 80 characters.");
        editName?.focus();
        return null;
    }
    /*
     ...
     */
    if (!PROFILE_USERNAME_REGEX.test(username)) {
        alert("Username must be 3–20 characters using lowercase letters, numbers or underscores.");
        editUsername?.focus();
        return null;
    }
    /*
    ...
    if (bio.length >
        ...
        alert(`Bio cannot be longer than ${PROFILE_BIO_MAX_LENGTH} characters.`);
        editBio?.focus();
        return null;
    }
    /*
...
async function saveProfile() {
     ...
     * Validate form.
     */
    const formData = validateProfileForm();
    if (!formData) {
        return false;
    }
    const userId = getUserId(currentUser);
    if (!userId) {
        alert("Your account ID is missing. Please log in again.");
        return false;
    }
    const currentUsername = getUsername(currentUser);
    /*
    ...
    try {
        ...
        if (formData.username !==
            currentUsername) {
            try {
                const availabilityResponse = await fetch(`${API_BASE}/check-username/${encodeURIComponent(formData.username)}?userId=${encodeURIComponent(userId)}`, {
                    method: "GET",
                    headers: {
                        "Accept": "application/json"
                    },
                ...
                const availabilityResult = await parseResponse(availabilityResponse);
                /*
                 ...
                 */
                console.warn("Username pre-check failed; continuing with profile save:", availabilityError);
            }
        }
        /*
         ...
         */
        window.dispatchEvent(new CustomEvent("dheere:profile-updated", {
            detail: {
                user: getCurrentUser()
            }
        }));
        ...
        alert(getErrorMessage(error, "Unable to update your profile."));
        return false;
    }
    finally {
        /*
...
function setupEditorEvents() {
     ...
     */
    if (editUsername) {
        editUsername.addEventListener("input", handleUsernameInput);
        editUsername.addEventListener("blur", () => {
            if (usernameCheckTimer !== null) {
                clearTimeout(usernameCheckTimer);
                usernameCheckTimer = null;
            }
     ...
     */
    if (editBio) {
        editBio.addEventListener("input", updateBioCounter);
    }
    /*
     * Save button.
     */
...
function initializeProfileEditor() {
    ...
    const avatarQuickEdit = document.getElementById("avatarQuickEdit");
    if (editProfileBtn) {
        editProfileBtn.addEventListener("click", openEditModal);
    }
    if (avatarQuickEdit) {
        avatarQuickEdit.addEventListener("click", openEditModal);
    }
    /*
...
/* =========================================================
   EXPORTS
   ========================================================= */
export { initializeProfileEditor, openEditModal, closeEditModal, saveProfile, checkUsernameAvailability, updateBioCounter, setUsernameStatus, isValidDateOfBirth };
