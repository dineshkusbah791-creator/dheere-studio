...
function getCurrentUser() {
    try {
        ...
        console.error("Community user data error:", error);
        clearAuthStorage();
        return null;
    }
}
...
function clearAuthStorage() {
    try {
        ...
        console.error("Could not clear community user storage:", error);
    }
    try {
        localStorage.removeItem(TOKEN_STORAGE_KEY);
    }
    catch (error) {
        console.error("Could not clear community token storage:", error);
    }
}
// ============================================================
// HANDLE AUTH ERROR
...
// ============================================================
async function handleAuthenticatedResponse(response) {
    if (response.status !==
        401) {
        return null;
    }
...
// MODULE EXPORTS
// ============================================================
export { USER_STORAGE_KEY, TOKEN_STORAGE_KEY, getAuthToken, getCurrentUser, getUsername, getUserId, hasValidLoginSession, getAuthHeaders, clearAuthStorage, handleAuthError, requireAuthentication, handleAuthenticatedResponse };
