...
const publicPostCount = document.getElementById("publicPostCount");
const publicProfileStatus = document.getElementById("publicProfileStatus");
const publicJoinedDate = document.getElementById("publicJoinedDate");
const publicPostsUsername = document.getElementById("publicPostsUsername");
const publicPostsLoading = document.getElementById("publicPostsLoading");
const publicPostsError = document.getElementById("publicPostsError");
const retryPostsBtn = document.getElementById("retryPostsBtn");
const publicPostsEmpty = document.getElementById("publicPostsEmpty");
...
function renderAvatar(user) {
    publicProfileAvatar.innerHTML =
        "";
    const avatarUrl = typeof user?.avatarUrl ===
        "string"
        ? user.avatarUrl.trim()
        : "";
    ...
    publicProfileAvatar.appendChild(image);
}
/* ============================================================
   PROFILE DATA
   ============================================================ */
function renderProfile(user, posts) {
    ...
    const name = typeof user.name ===
        ...
        ? user.name.trim()
        : "";
    const username = typeof user.username ===
        "string"
        ? user.username.trim()
        : "";
    const bio = typeof user.bio ===
        "string"
        ? user.bio.trim()
        : "";
...
async function fetchPublicProfile() {
    ...
    const query = viewerId
        ? `?userId=${encodeURIComponent(viewerId)}`
        : "";
    const url = `${API_BASE}/public-profile/${encodeURIComponent(currentUsername)}${query}`;
...
/* ============================================================
   LOAD COMMENTS
   ============================================================ */
async function loadComments(postId, commentsList, countElement) {
    if (!postId ||
        !commentsList) {
        return;
    }
    ...
    try {
        ...
        console.error("Live profile comments error:", error);
        commentsList.innerHTML = `
...
/* ============================================================
   ADD COMMENT
   ============================================================ */
async function addComment(postId, input, submitButton, commentsList, countElement) {
    ...
    try {
        ...
        input.value =
            "";
        if (result?.comment) {
            await loadComments(postId, commentsList, countElement);
        }
        else {
            await loadComments(postId, commentsList, countElement);
        }
        ...
        console.error("Add live profile comment error:", error);
        alert(error.message ||
...
async function toggleLike(postId, button, countElement) {
    ...
    try {
        ...
        button.setAttribute("aria-pressed", liked
            ? "true"
            : "false");
        const icon = button.querySelector(".live-post-action-icon");
        const text = button.querySelector(".live-post-action-text");
        if (icon) {
            icon.textContent =
                liked
                    ? "♥"
        ...
        console.error("Live profile like error:", error);
        alert(error.message ||
...
function renderPosts(posts) {
    ...
    publicPostsFeed.appendChild(fragment);
}
/* ============================================================
   CREATE POST
   ============================================================ */
function createPostElement(post) {
    ...
    author.className =
        "public-post-author";
    const username = typeof post?.username ===
        "string"
        ? post.username.trim()
        : currentUsername;
    author.textContent =
...
/* ============================================================
   ...
   ============================================================ */
if (publicPostsFeed) {
    publicPostsFeed.addEventListener("click", async (event) => {
        /* ----------------------------------------
           LIKE
           ---------------------------------------- */
        const likeButton = event.target instanceof Element
            ? event.target.closest(".live-post-like-button")
            : null;
        if (likeButton) {
            const postCard = likeButton.closest(".public-post-card");
            const postId = likeButton.dataset.postId;
            const countElement = postCard?.querySelector(".live-post-like-count");
            if (postId &&
                countElement) {
        ...
        /* ----------------------------------------
           COMMENT TOGGLE
           ---------------------------------------- */
        const commentButton = event.target instanceof Element
            ? event.target.closest(".live-post-comment-button")
            : null;
        if (commentButton) {
            const postCard = commentButton.closest(".public-post-card");
            if (!postCard) {
                return;
            }
            const postId = commentButton.dataset.postId;
            const panel = postCard.querySelector(".live-post-comments-panel");
            const commentsList = postCard.querySelector(".live-comments-list");
            const countElement = postCard.querySelector(".live-post-comment-count");
            if (!postId ||
                !panel ||
                !commentsList) {
                return;
            ...
            await loadComments(postId, commentsList, countElement);
            const input = postCard.querySelector(".live-comment-input");
            if (input) {
                input.focus();
            }
            return;
        ...
        const submitButton = event.target instanceof Element
            ? event.target.closest(".live-submit-comment")
            : null;
        if (submitButton) {
            const postCard = submitButton.closest(".public-post-card");
            if (!postCard) {
                return;
            }
            ...
            if (postId &&
                ...
                commentsList &&
                countElement) {
                await addComment(postId, input, submitButton, commentsList, countElement);
            }
        }
    });
    /* ----------------------------------------
       ENTER TO COMMENT
       ---------------------------------------- */
    publicPostsFeed.addEventListener("keydown", event => {
        if (event.key !==
            "Enter" ||
            event.shiftKey) {
            return;
        ...
        const input = event.target instanceof Element
            ? event.target.closest(".live-comment-input")
            : null;
        if (!input) {
            return;
        }
        event.preventDefault();
        const postCard = input.closest(".public-post-card");
        const submitButton = postCard?.querySelector(".live-submit-comment");
        if (submitButton) {
            submitButton.click();
        }
    });
...
/* ============================================================
   ...
   ============================================================ */
if (retryPostsBtn) {
    retryPostsBtn.addEventListener("click", () => {
        loadProfile();
    });
}
/* ============================================================
...
function initializeLiveProfile() {
    ...
    if (!validateUsername(currentUsername)) {
        showProfileError("The profile username in this URL is invalid.");
        return;
    }
    loadProfile();
}
...
if (document.readyState ===
    "loading") {
    document.addEventListener("DOMContentLoaded", initializeLiveProfile, {
        once: true
    });
}
else {
