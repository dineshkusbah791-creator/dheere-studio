...
const PROFILE_POSTS_PATH = 'profile/profile.html#postsSection';
const AUTH_CHANGED_EVENT = 'dheere:auth-changed';
const HOMEPAGE_REFRESH_EVENT = 'dheere:homepage-personalization-refresh';
// ======================================================
// AUTH TOKEN HELPERS
// ======================================================
...
if (goToLoginBtn) {
    goToLoginBtn.addEventListener('click', openCanonicalLogin);
}
// ======================================================
...
let profileMenuWrapper = document.getElementById('profileMenuWrapper');
let profileDropdown = document.getElementById('profileDropdown');
let profileMenuProfile = document.getElementById('profileMenuProfile');
let profileMenuPosts = document.getElementById('profileMenuPosts');
let profileMenuSettings = document.getElementById('profileMenuSettings');
let profileMenuLogout = document.getElementById('profileMenuLogout');
let navUsername = document.getElementById('navUsername');
let navProfileAvatar = document.getElementById('navProfileAvatar');
// ======================================================
...
function restoreUserFromStorage() {
    try {
        ...
        console.error('Saved user data error:', error);
        currentUser =
            null;
        clearAuthStorage();
    }
...
function updateHomepagePersonalization() {
     ...
     */
    const publicTitle = 'Welcome to Dheere Studio.A Studio of Stories & World.';
    /*
     * Logged out state.
     */
    if (!currentUser) {
...
function syncHomepageAuthentication() {
    const nextSnapshot = getHomepageAuthSnapshot();
    if (nextSnapshot ===
...
homepageAuthSnapshot =
    getHomepageAuthSnapshot();
updateHomepagePersonalization();
window.addEventListener('storage', refreshHomepageAuthenticationUI);
window.addEventListener('pageshow', refreshHomepageAuthenticationUI);
window.addEventListener(AUTH_CHANGED_EVENT, refreshHomepageAuthenticationUI);
window.addEventListener(HOMEPAGE_REFRESH_EVENT, updateHomepagePersonalization);
/* ======================================================
...
function applyProfileToCurrentUser(profile) {
    ...
    try {
        ...
        console.error('Could not save current user state:', error);
    }
}
// ======================================================
...
function getUserInitials(user) {
    const name = String(getUserName(user) || 'User').trim();
    if (!name) {
        return 'U';
    }
...
// ======================================================
function renderAvatarElement(container, avatarUrl, user) {
    if (!container) {
        return;
    }
...
function renderHomepageAvatar() {
    ...
    const avatarUrl = currentUser?.avatarUrl ||
        '';
    renderAvatarElement(navProfileAvatar, avatarUrl, currentUser);
}
// ======================================================
// HTML ESCAPE
// ======================================================
...
function updateNavbar() {
    ...
    if (!currentUser) {
        ...
        if (profileMenuWrapper) {
            profileMenuWrapper.style.display =
                'none';
            profileMenuWrapper.classList.add('hidden-profile');
        }
        else {
        ...
        closeProfileMenu();
        if (typeof stopNotificationRefresh ===
            'function') {
            stopNotificationRefresh();
        }
        if (typeof resetNotificationsUI ===
            'function') {
    ...
    if (profileMenuWrapper) {
        profileMenuWrapper.style.display =
            'flex';
        profileMenuWrapper.classList.remove('hidden-profile');
    }
    profileBtn.style.display =
        'flex';
    profileBtn.classList.remove('hidden-profile');
    profileBtn.classList.remove('button');
    profileBtn.classList.add('nav-profile-link');
    ...
    if (typeof initializeNotificationUI ===
        'function') {
        initializeNotificationUI();
    }
    if (typeof startNotificationRefresh ===
        'function') {
...
function isProfileMenuOpen() {
    return Boolean(profileDropdown &&
...
function handleProfileDropdownKeydown(event) {
    if (!profileDropdown) {
        return;
    }
    const items = Array.from(profileDropdown.querySelectorAll('[role="menuitem"]:not([disabled])'));
    if (!items.length) {
        return;
    }
    ...
    if (event.key ===
        ...
        items[currentIndex < 0
            ? 0
            : (currentIndex + 1) % items.length].focus();
        return;
    }
    if (event.key ===
        'ArrowUp') {
...
function logoutHomepageUser() {
    ...
    try {
        ...
        console.error('Could not dispatch auth change event:', error);
    }
}
function initializeProfileMenu() {
    ...
    profileMenuReady =
        true;
    profileBtn.addEventListener('click', handleProfileMenuClick);
    profileBtn.addEventListener('keydown', handleProfileMenuKeydown);
    profileDropdown.addEventListener('keydown', handleProfileDropdownKeydown);
    profileMenuLogout?.addEventListener('click', logoutHomepageUser);
    /*
    ...
    if (profileMenuPosts) {
        profileMenuPosts.href =
            PROFILE_POSTS_PATH;
        profileMenuPosts.addEventListener('click', () => closeProfileMenu());
    }
    /*
     * My Profile and Settings remain normal links.
     * No URL replacement is performed here.
     */
    profileMenuProfile?.addEventListener('click', () => closeProfileMenu());
    profileMenuSettings?.addEventListener('click', () => closeProfileMenu());
    document.addEventListener('click', (event) => {
        if (!profileMenuWrapper ||
            profileMenuWrapper.contains(event.target)) {
            return;
        }
        closeProfileMenu();
    });
...
if (usernameInput) {
    usernameInput.addEventListener('input', () => {
        ...
        usernameCheckTimer =
            setTimeout(checkUsernameAvailability, 400);
    });
}
// ======================================================
if (registerForm) {
        ...
        try {
            ...
            if (result.success) {
                ...
                if (!result.token ||
                    typeof result.token !==
                        'string') {
                    console.error('Login successful but JWT token is missing.');
                    alert('Login token was not received. Please try again.');
                    return;
                }
                // ------------------------------------------
                // SAVE CURRENT USER
            ...
            alert('Backend server se connect nahi ho paya. Please try again.');
        }
    });
}
// ======================================================
...
async function loadProfilePage() {
    ...
    if (profileAvatar) {
        renderAvatarElement(profileAvatar, user?.avatarUrl || '', user);
    }
    profileCard.style.display =
        'block';
    if (postsSection) {
...
if (createPostBtn) {
    createPostBtn.addEventListener('click', async () => {
        ...
        if (content.length >
            2000) {
            alert('Post cannot exceed 2000 characters.');
            return;
        }
        // ----------------------------------------------
        // LOADING STATE
