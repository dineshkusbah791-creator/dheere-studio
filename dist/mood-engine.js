...
(function () {
    ...
    const TRANSITION_TIME = 1150;
    const EXIT_BUFFER = 120;
    const BOOT_DELAY = 60;
    const MAX_LAYERS = 2;
    ...
    function applyRandomDelay(element, min, max) {
        if (prefersReducedMotion()) {
            return;
        }
    ...
    function ensureEnvironmentRoot() {
        environmentRoot =
            document.getElementById("dheereEnvironment");
        if (environmentRoot) {
        ...
        environmentRoot.id =
            "dheereEnvironment";
        environmentRoot.setAttribute("aria-hidden", "true");
        Object.assign(environmentRoot.style, {
            position: "fixed",
            inset: "0",
            zIndex: "0",
            pointerEvents: "none",
        ...
        document.body.prepend(environmentRoot);
        return environmentRoot;
    }
    /* ============================================================
       LAYER MANAGEMENT
       ============================================================ */
    function createLayer(mood) {
        const root = ensureEnvironmentRoot();
    ...
    function cleanupInactiveLayers(activeLayer) {
        if (!environmentRoot) {
            return;
        }
        Array.from(environmentRoot.children).forEach(function (layer) {
            if (layer !==
                activeLayer) {
                fadeOutLayer(layer);
            }
    ...
    function trimLayerStack() {
        ...
        const layers = Array.from(environmentRoot.children);
        if (layers.length <=
            MAX_LAYERS) {
            return;
        }
        const excess = layers.length -
            MAX_LAYERS;
        layers
            .slice(0, excess)
    ...
    function injectCinematicAtmosphereCSS() {
        if (document.getElementById("dheere-cinematic-atmosphere-v6"))
            return;
    ...
    function injectWorldDirectorCSS() {
        if (document.getElementById("dheere-world-director-v6"))
            return;
        const style = document.createElement("style");
    ...
    /* ============================================================
       ORIGINAL
       ============================================================ */
    function buildOriginal(layer) {
        layer.dataset.environment =
            "original";
        const original = createElement("original-environment", layer);
        createElement("original-ambient", original);
        createElement("original-light", original);
        createElement("original-grain", original);
    }
    function buildForest(layer) {
        const forest = createElement("forest-environment", layer);
        ...
        createElement("forest-canopy left", forest);
        createElement("forest-canopy right", forest);
        const branches = [
            {
                side: "left",
                top: "8%",
        ...
        branches.forEach(function (data) {
            const branch = createElement("forest-branch", forest);
            if (data.side ===
                "left") {
                branch.style.left =
                    "-48px";
        ...
        if (scale === 0) {
            createElement("forest-mist", forest);
            return;
        }
        const leafCount = Math.max(6, Math.round(24 * scale));
        for (let i = 0; i < leafCount; i++) {
            const leaf = createElement("forest-leaf", forest);
            const leftSide = i %
                2 ===
                0;
            leaf.style.left =
            ...
            leaf.style.top =
                random(0, 88) + "%";
            setVar(leaf, "--leaf-x", random(-4, 20) + "px");
            setVar(leaf, "--leaf-y", random(6, 18) + "px");
            setVar(leaf, "--leaf-speed", random(8, 16) + "s");
            leaf.style.opacity =
                random(.24, .64);
            applyRandomDelay(leaf, 0, 14);
        }
        createElement("forest-mist", forest);
    }
    /* ============================================================
       OCEAN
       ============================================================ */
    function buildOcean(layer) {
        const ocean = createElement("ocean-environment", layer);
        createElement("ocean-depth", ocean);
        createElement("ocean-horizon", ocean);
        createElement("ocean-caustic", ocean);
        const waveCount = isSmallMobile()
            ? 5
            : isMobile()
                ? 6
                : 8;
        for (let i = 0; i < waveCount; i++) {
            const wave = createElement("ocean-wave", ocean);
            wave.style.top =
                (15 +
                    i * 11) + "%";
            const depth = i /
            ...
            wave.style.opacity =
                clamp(.38 -
                    depth * .22 +
                    random(-.03, .03), .10, .38);
            setVar(wave, "--wave-speed", random(10, 18) + "s");
            applyRandomDelay(wave, 0, 12);
        }
        const rayCount = isMobile()
            ? 2
            : 4;
        for (let i = 0; i < rayCount; i++) {
            const ray = createElement("ocean-ray", ocean);
            ray.style.left =
                (5 +
                    i *
                        (90 /
                            Math.max(rayCount -
                                1, 1))) + "%";
            setVar(ray, "--ray-speed", random(11, 18) + "s");
            ray.style.opacity =
                random(.08, .18);
            applyRandomDelay(ray, 0, 14);
        }
        const scale = motionScale();
        if (scale === 0) {
            return;
        }
        const particleCount = Math.max(5, Math.round(20 * scale));
        for (let i = 0; i < particleCount; i++) {
            const particle = createElement("ocean-particle", ocean);
            particle.style.left =
                random(2, 98) + "%";
            particle.style.top =
                random(42, 100) + "%";
            setVar(particle, "--particle-speed", random(9, 19) + "s");
            setVar(particle, "--particle-x", random(-30, 30) + "px");
            particle.style.opacity =
                random(.15, .50);
            applyRandomDelay(particle, 0, 15);
        }
    }
    /* ============================================================
       EMBER
       ============================================================ */
    function buildEmber(layer) {
        const ember = createElement("ember-environment", layer);
        createElement("ember-heat", ember);
        createElement("ember-core", ember);
        const glow = createElement("ember-glow", ember);
        setVar(glow, "--ember-intensity", "1");
        const secondGlow = createElement("ember-glow", ember);
        ...
        const sparkCount = Math.max(5, Math.round(24 * scale));
        for (let i = 0; i < sparkCount; i++) {
            const spark = createElement("ember-spark", ember);
            spark.style.left =
                random(8, 92) + "%";
            spark.style.bottom =
                random(-8, 12) + "%";
            setVar(spark, "--spark-size", random(1.2, 3.8) + "px");
            setVar(spark, "--spark-speed", random(5, 10) + "s");
            setVar(spark, "--spark-x", random(-38, 38) + "px");
            spark.style.opacity =
                random(.28, .78);
            applyRandomDelay(spark, 0, 12);
        }
    }
    /* ============================================================
       MIDNIGHT
       ============================================================ */
    function buildMidnight(layer) {
        const midnight = createElement("midnight-environment", layer);
        createElement("midnight-nebula", midnight);
        createElement("midnight-halo", midnight);
        createElement("midnight-moon", midnight);
        const scale = motionScale();
        if (scale === 0) {
            return;
        }
        const starCount = Math.max(24, Math.round(72 * scale));
        for (let i = 0; i < starCount; i++) {
            const star = createElement("midnight-star", midnight);
            star.style.left =
                random(1, 99) + "%";
            star.style.top =
                random(1, 98) + "%";
            setVar(star, "--star-size", random(.7, 2.5) + "px");
            setVar(star, "--star-opacity", random(.18, .74));
            setVar(star, "--star-min", random(.04, .17));
            setVar(star, "--star-speed", random(3.8, 8) + "s");
            applyRandomDelay(star, 0, 10);
        }
        const particleCount = Math.max(4, Math.round(12 * scale));
        for (let i = 0; i < particleCount; i++) {
            const particle = createElement("midnight-particle", midnight);
            particle.style.left =
                random(4, 96) + "%";
            particle.style.top =
                random(16, 96) + "%";
            setVar(particle, "--float-x", random(-32, 32) + "px");
            setVar(particle, "--float-y", random(-30, 26) + "px");
            setVar(particle, "--particle-speed", random(12, 22) + "s");
            particle.style.opacity =
                random(.08, .30);
            applyRandomDelay(particle, 0, 15);
        }
    ...
    /* ============================================================
       PRISM
       ============================================================ */
    function buildPrism(layer) {
        injectPrismEnvironmentCSS();
        const prism = createElement("prism-environment-v2", layer);
        createElement("prism-air", prism);
        createElement("prism-spectrum-field", prism);
        createElement("prism-light-band one", prism);
        createElement("prism-light-band two", prism);
        createElement("prism-light-band three", prism);
        createElement("prism-refraction one", prism);
        createElement("prism-refraction two", prism);
        createElement("prism-refraction three", prism);
        const scale = motionScale();
        if (scale === 0) {
            return;
        }
        const particleCount = Math.max(8, Math.round(24 * scale));
        ...
        for (let i = 0; i < particleCount; i++) {
            const particle = createElement("prism-atmosphere-particle", prism);
            particle.style.left =
                random(4, 96) + "%";
            particle.style.top =
                random(8, 92) + "%";
            setVar(particle, "--size", random(1, 3.5) + "px");
            setVar(particle, "--opacity", random(.16, .48));
            setVar(particle, "--drift-x", random(-28, 28) + "px");
            setVar(particle, "--drift-y", random(-24, 24) + "px");
            setVar(particle, "--speed", random(11, 20) + "s");
            setVar(particle, "--particle-color", particleColors[i %
    ...
    /* ============================================================
       DESERT
       ============================================================ */
    function buildDesert(layer) {
        const desert = createElement("desert-environment", layer);
        createElement("desert-sun-halo", desert);
        createElement("desert-shimmer", desert);
        const sky = createElement("desert-sky", desert);
        const sun = createElement("desert-sun", desert);
        const horizon = createElement("desert-horizon", desert);
        const dunesFar = createElement("desert-dunes-far", desert);
        const dunesMid = createElement("desert-dunes-mid", desert);
        const haze = createElement("desert-haze", desert);
        const wind = createElement("desert-wind", desert);
        const gusts = createElement("desert-gusts", desert);
        setStyles(sky, {
            position: "absolute",
            inset: "0",
            background: "linear-gradient(180deg, rgba(255,255,246,.98) 0%, rgba(255,249,219,.90) 12%, rgba(255,231,175,.50) 30%, rgba(239,212,164,.16) 48%, transparent 74%, rgba(89,68,46,.045) 100%)",
            pointerEvents: "none"
        });
        setStyles(sun, {
            position: "absolute",
            ...
            height: "74px",
            borderRadius: "50%",
            background: "radial-gradient(circle at 35% 32%, #ffffff 0%, #fffdf0 23%, #fff4bf 46%, #ffe08a 68%, rgba(246,197,103,0) 100%)",
            boxShadow: "0 0 34px rgba(255,248,208,.62), 0 0 92px rgba(255,218,139,.30)",
            opacity: ".98",
            pointerEvents: "none"
        });
        setStyles(horizon, {
            ...
            bottom: "34%",
            height: "2px",
            background: "linear-gradient(90deg, transparent, rgba(222,193,152,.14), rgba(177,148,109,.18), transparent)",
            filter: "blur(.7px)",
            opacity: ".82"
        });
        setStyles(dunesFar, {
            ...
            bottom: "19%",
            height: "36%",
            background: "radial-gradient(ellipse at 18% 82%, rgba(173,145,106,.16), transparent 34%), radial-gradient(ellipse at 61% 76%, rgba(157,128,91,.12), transparent 36%), radial-gradient(ellipse at 88% 88%, rgba(132,105,76,.11), transparent 29%)",
            filter: "blur(4px)"
        });
        setStyles(dunesMid, {
            position: "absolute",
            ...
            bottom: "5%",
            height: "34%",
            background: "radial-gradient(ellipse at 12% 92%, rgba(119,95,66,.15), transparent 37%), radial-gradient(ellipse at 54% 84%, rgba(136,105,72,.17), transparent 39%), radial-gradient(ellipse at 89% 95%, rgba(103,80,55,.14), transparent 33%)",
            filter: "blur(1.5px)"
        });
        setStyles(haze, {
            position: "absolute",
            ...
            bottom: "16%",
            height: "31%",
            background: "radial-gradient(ellipse at center, rgba(222,199,167,.045), transparent 68%)",
            filter: "blur(24px)",
            opacity: ".72"
        });
        setStyles(gusts, {
            position: "absolute",
            inset: "-20%",
            background: "linear-gradient(180deg, transparent 0 44%, rgba(224,203,174,.045) 47%, transparent 51% 100%)",
            filter: "blur(11px)",
        ...
        const dustCount = Math.max(12, Math.round(52 * scale));
        for (let i = 0; i < dustCount; i++) {
            const dust = createElement("desert-dust-particle", desert);
            dust.style.left =
                random(-8, 100) + "%";
            dust.style.top =
                random(48, 94) + "%";
            setVar(dust, "--dust-speed", random(2.9, 6.8) + "s");
            setVar(dust, "--dust-drift", random(60, 150) + "px");
            setVar(dust, "--dust-rise", random(-4, -24) + "px");
            setVar(dust, "--dust-size", random(.65, 2.2) + "px");
            dust.style.opacity =
                random(.10, .34);
            applyRandomDelay(dust, 0, 7);
        }
        const fineCount = Math.max(6, Math.round(16 * scale));
        for (let i = 0; i < fineCount; i++) {
            const fine = createElement("desert-dust-fine", desert);
            fine.style.left =
                random(-4, 102) + "%";
            fine.style.top =
                random(35, 86) + "%";
            setVar(fine, "--fine-speed", random(5, 10) + "s");
            setVar(fine, "--fine-drift", random(80, 190) + "px");
            setVar(fine, "--fine-size", random(.5, 1.35) + "px");
            fine.style.opacity =
                random(.08, .22);
            applyRandomDelay(fine, 0, 10);
        }
    }
    /* ============================================================
       MONSOON
       ============================================================ */
    function buildMonsoon(layer) {
        const monsoon = createElement("monsoon-environment", layer);
        createElement("monsoon-depth", monsoon);
        createElement("monsoon-wet-glow", monsoon);
        const clouds = createElement("monsoon-clouds", monsoon);
        const drops = createElement("monsoon-drops", monsoon);
        const ground = createElement("monsoon-ground", monsoon);
        const lightning = createElement("monsoon-lightning", monsoon);
        setStyles(clouds, {
            position: "absolute",
            inset: "0",
            background: "radial-gradient(ellipse at 22% 16%, rgba(78,94,103,.19), transparent 32%), radial-gradient(ellipse at 72% 18%, rgba(66,84,93,.18), transparent 36%), radial-gradient(ellipse at 50% 42%, rgba(43,57,63,.10), transparent 50%), linear-gradient(180deg, rgba(48,61,68,.08), transparent 70%)",
            filter: "blur(28px)",
            animation: prefersReducedMotion()
                ? "none"
        ...
        setStyles(ground, {
            ...
            bottom: "0",
            height: "31%",
            background: "linear-gradient(180deg, transparent, rgba(28,63,67,.30) 58%, rgba(11,30,33,.44))",
            overflow: "hidden"
        });
        setStyles(lightning, {
            position: "absolute",
            inset: "0",
            background: "rgba(225,242,250,.13)",
            opacity: "0",
            animation: prefersReducedMotion()
                ? "none"
        ...
        const dropCount = Math.max(18, Math.round(72 * scale));
        for (let i = 0; i < dropCount; i++) {
            const drop = createElement("monsoon-drop", drops);
            drop.style.left =
                random(-3, 103) + "%";
            drop.style.top =
                random(-18, 94) + "%";
            setVar(drop, "--drop-length", random(18, 42) + "px");
            setVar(drop, "--drop-width", random(1.8, 3.2) + "px");
            setVar(drop, "--drop-speed", random(.55, 1.25) + "s");
            setVar(drop, "--drop-drift", random(18, 52) + "px");
            drop.style.opacity =
                random(.20, .68);
            applyRandomDelay(drop, 0, 4);
        }
        const foregroundCount = Math.max(5, Math.round(16 * scale));
        for (let i = 0; i < foregroundCount; i++) {
            const drop = createElement("monsoon-drop-foreground", drops);
            drop.style.left =
                random(0, 100) + "%";
            drop.style.top =
                random(-10, 85) + "%";
            setVar(drop, "--drop-fg-speed", random(.9, 1.8) + "s");
            setVar(drop, "--drop-fg-drift", random(18, 42) + "px");
            setVar(drop, "--drop-fg-height", random(28, 58) + "px");
            setVar(drop, "--drop-fg-width", random(2.4, 4.2) + "px");
            drop.style.opacity =
                random(.28, .72);
            applyRandomDelay(drop, 0, 5);
        }
        const microDropCount = Math.max(28, Math.round(96 * scale));
        for (let i = 0; i < microDropCount; i++) {
            const drop = createElement("monsoon-drop-micro", drops);
            drop.style.left =
                random(-4, 104) + "%";
            drop.style.top =
                random(-14, 100) + "%";
            setVar(drop, "--micro-drop-size", random(.9, 1.7) + "px");
            setVar(drop, "--micro-drop-length", random(10, 22) + "px");
            setVar(drop, "--micro-drop-speed", random(.24, .62) + "s");
            setVar(drop, "--micro-drop-drift", random(8, 26) + "px");
            drop.style.opacity =
                random(.14, .42);
            applyRandomDelay(drop, 0, 2.5);
        }
        const rippleCount = Math.max(6, Math.round(18 * scale));
        for (let i = 0; i < rippleCount; i++) {
            const ripple = createElement("monsoon-ripple", ground);
            ripple.style.left =
                random(4, 96) + "%";
            ripple.style.bottom =
                random(2, 20) + "%";
            setVar(ripple, "--ripple-delay", random(0, 5) + "s");
            setVar(ripple, "--ripple-size", random(12, 42) + "px");
        }
    }
    /* ============================================================
       WINTER
       ============================================================ */
    function buildWinter(layer) {
        const winter = createElement("winter-environment", layer);
        createElement("winter-depth", winter);
        createElement("winter-glow-line", winter);
        const frost = createElement("winter-frost", winter);
        const ground = createElement("winter-ground", winter);
        const snow = createElement("winter-snow", winter);
        const iceDust = createElement("winter-ice-dust", winter);
        const wind = createElement("winter-wind", winter);
        const iceCrystals = createElement("winter-ice-crystals", winter);
        setStyles(frost, {
            position: "absolute",
            inset: "0",
            background: "radial-gradient(ellipse at 50% 0%, rgba(194,226,241,.18), transparent 54%), radial-gradient(ellipse at 20% 70%, rgba(173,214,232,.05), transparent 42%)",
            filter: "blur(2px)",
            opacity: ".88"
        });
        setStyles(ground, {
            ...
            bottom: "0",
            height: "28%",
            background: "linear-gradient(180deg, transparent, rgba(217,239,249,.09) 50%, rgba(204,229,241,.16))",
            filter: "blur(2px)"
        });
        setStyles(snow, {
            position: "absolute",
        ...
        setStyles(iceDust, {
            ...
            opacity: prefersReducedMotion()
                ? ".11"
                : ".24",
            background: "radial-gradient(ellipse at center, rgba(210,237,247,.05), transparent 70%)",
        ...
        setStyles(wind, {
            ...
            opacity: prefersReducedMotion()
                ? ".08"
                : ".19",
            background: "radial-gradient(ellipse at center, rgba(222,244,251,.05), transparent 72%)",
            transform: "rotate(-6deg)",
            animation: prefersReducedMotion()
                ? "none"
                : "mood-winter-wind 3.4s linear infinite"
        });
        setStyles(iceCrystals, {
            position: "absolute",
            inset: "-15%",
            background: "radial-gradient(ellipse at center, rgba(215,242,251,.045), transparent 72%)",
            opacity: prefersReducedMotion()
                ? ".08"
                : ".18",
            animation: prefersReducedMotion()
        ...
        /* --------------------------------------------------------
           ...
           -------------------------------------------------------- */
        const iceCount = Math.max(24, Math.round(90 * scale));
        for (let i = 0; i < iceCount; i++) {
            const crystal = createElement("winter-ice-particle", winter);
            crystal.style.left =
                random(-5, 105) + "%";
            crystal.style.top =
                random(-10, 96) + "%";
            setVar(crystal, "--ice-speed", random(3.0, 7.6) + "s");
            setVar(crystal, "--ice-drift", random(-55, 55) + "px");
            const crystalRoll = Math.random();
            setVar(crystal, "--ice-size", crystalRoll < .62
                ? random(3.5, 7.5) + "px"
                : crystalRoll < .90
                    ? random(7.5, 12.5) + "px"
                    : random(12, 19) + "px");
            setVar(crystal, "--ice-width", crystalRoll < .62
                ? random(1.5, 3.6) + "px"
                : crystalRoll < .90
                    ? random(2.8, 5.6) + "px"
                    : random(4.8, 8.4) + "px");
            setVar(crystal, "--ice-fall", random(88, 135) + "vh");
            setVar(crystal, "--ice-rotation", random(-40, 40) + "deg");
    ...
    function injectSeasonalAnimationCSS() {
        if (document.getElementById("dheere-seasonal-engine-v1")) {
            return;
        }
        const style = document.createElement("style");
        style.id =
    ...
    /* ============================================================
       BUILD DISPATCHER
       ============================================================ */
    function populateLayer(mood, layer) {
        switch (mood) {
            case "forest":
                buildForest(layer);
                break;
    ...
    /* ============================================================
       ...
       ============================================================ */
    function buildWorldDepth(layer, mood) {
        const depth = createElement("world-depth", layer);
        ...
        const focus = createElement("world-focus", depth);
        focus.dataset.mood = mood;
        setVar(focus, "--world-seed", random(0, 1).toFixed(3));
        if (mood === "forest") {
    ...
    function applyMood(mood, options = {}) {
        ...
        if (mood ===
            "original") {
            const oldLayers = Array.from(environmentRoot.children);
            if (prefersReducedMotion()) {
                clearEnvironment();
                const originalLayer = createLayer("original");
                originalLayer.style.opacity = "1";
                populateLayer("original", originalLayer);
                buildWorldDepth(originalLayer, "original");
                addWorldLivingDetails(originalLayer, "original");
                return;
            }
        ...
        const newLayer = createLayer(mood);
        populateLayer(mood, newLayer);
        buildWorldDepth(newLayer, mood);
        addWorldLivingDetails(newLayer, mood);
        trimLayerStack();
        if (prefersReducedMotion()) {
            cleanupInactiveLayers(newLayer);
            return;
        }
        cleanupTimer =
            window.setTimeout(function () {
    ...
    function syncMoodUI(mood) {
        const cards = document.querySelectorAll("[data-mood]");
        cards.forEach(function (card) {
            const cardMood = card.dataset.mood;
            const active = cardMood ===
                mood;
    ...
    function bindMoodCards() {
        const cards = document.querySelectorAll("[data-mood]");
        cards.forEach(function (card) {
            if (card.dataset.dheereMoodBound ===
                "true") {
                return;
            }
    ...
    function syncEnvironmentPlayback() {
        ...
        environmentRoot.classList.toggle("mood-animations-paused", document.visibilityState !==
            "visible");
    }
    document.addEventListener("visibilitychange", syncEnvironmentPlayback);
    /* ============================================================
       SYNCHRONIZATION
       ============================================================ */
    ...
    window.addEventListener("pageshow", syncSavedMood);
    /* ============================================================
       RESIZE
       ============================================================ */
    ...
    if (document.readyState ===
        "loading") {
        document.addEventListener("DOMContentLoaded", initializeMood, {
            once: true
        });
    }
    else {
