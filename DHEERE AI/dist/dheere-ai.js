...
/* =========================================================
   STATUS
   ========================================================= */
function setAIStatus(text, online = false) {
    if (aiStatusText) {
...
function setTyping(visible) {
    ...
    aiTyping.classList.toggle("visible", visible);
}
/* =========================================================
   BUTTON STATE
   ========================================================= */
...
function createAIInterface() {
     ...
     * a Dheere AI widget.
     */
    if (document.getElementById("dheereAiRoot")) {
        aiTrigger =
            document.getElementById("dheereAiTrigger");
        aiPanel =
            document.getElementById("dheereAiPanel");
        aiCloseButton =
            document.getElementById("dheereAiClose");
        aiMessages =
            document.getElementById("dheereAiMessages");
        aiInput =
            document.getElementById("dheereAiInput");
        aiSendButton =
            document.getElementById("dheereAiSend");
        aiTyping =
            document.getElementById("dheereAiTyping");
        aiStatus =
            document.getElementById("dheereAiStatus");
        aiStatusText =
            document.getElementById("dheereAiStatusText");
        return;
    }
    /*
     * Root container.
...
function openAI() {
    ...
    aiTrigger.setAttribute("aria-label", "Close Dheere AI");
    setTimeout(() => {
        aiInput?.focus();
    }, 120);
}
...
function closeAI() {
    ...
    aiTrigger.setAttribute("aria-label", "Open Dheere AI");
}
/* =========================================================
   TOGGLE PANEL
   ========================================================= */
...
function addMessage(text, type) {
    ...
    if (aiTyping) {
        aiMessages.insertBefore(message, aiTyping);
    }
    else {
     ...
     */
    while (aiMessageCount >
        MAX_VISIBLE_MESSAGES) {
        const candidates = aiMessages.querySelectorAll(".dheere-ai-message");
...
function resizeInput() {
    ...
    aiInput.style.height =
        "auto";
    aiInput.style.height =
        Math.min(aiInput.scrollHeight, 140) + "px";
}
/* =========================================================
   CLEAR INPUT
   ========================================================= */
...
async function sendMessage() {
    ...
    try {
        ...
        if (typeof result?.answer !==
            "string" ||
            !result.answer.trim()) {
            throw new Error("Dheere AI returned an empty response.");
        }
        /*
         * AI response.
         */
        addMessage(result.answer.trim(), "ai");
        setAIStatus("Ready", true);
    }
    catch (error) {
        console.error("Dheere AI error:", error);
...
function setupEvents() {
    ...
    if (aiCloseButton) {
        aiCloseButton.addEventListener("click", (event) => {
            event.preventDefault();
            closeAI();
        });
    }
     ...
     * Form submit.
     */
    const form = document.getElementById("dheereAiForm");
    if (form) {
    ...
    document.addEventListener("click", (event) => {
        ...
        const clickedInsidePanel = aiPanel.contains(target);
        const clickedTrigger = aiTrigger?.contains(target) ?? false;
...
if (document.readyState ===
    "loading") {
    document.addEventListener("DOMContentLoaded", initializeDheereAI, {
        once: true
    });
}
else {
...
/* =========================================================
   EXPORTS
   ========================================================= */
export { initializeDheereAI, openDheereAI, closeDheereAI, toggleDheereAI, sendDheereAIMessage };
//# sourceMappingURL=dheere-ai.js.map